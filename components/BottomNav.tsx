...
const navItems: { view: View; label: string; icon: React.FC<React.SVGProps<SVGSVGElement>> }[] = [
  ...
  { view: 'daily', label: 'Daily', icon: SparklesIcon },
  { view: 'predictions', label: 'Predictions', icon: ChartBarIcon },
...
const NavItem: React.FC<{
    ...
    return (
        ...
        >
            ...
            <span className={`text-xs mt-1 transition-opacity duration-300 ${isActive ? 'opacity-100 font-semibold' : 'opacity-70'}`}>
                {item.label}
            </span>
        </button>
    );
...
export const BottomNav: React.FC<BottomNavProps> = ({ activeView, setActiveView }) => {
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-20 bg-black/30 backdrop-blur-xl border-t border-white/10 z-20">
