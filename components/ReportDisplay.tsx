...
const NumberCard: React.FC<{ title: string; detail: CalculationDetail; keywords: string }> = ({ title, detail, keywords }) => {
    ...
    return (
        <Card 
            ref={ref} 
            className={`flex flex-col p-0 overflow-hidden transition-all duration-700 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
        >
            <button
                onClick={() => {
...
const InterpretationCard: React.FC<{ title: string; interpretation: Interpretation }> = ({ title, interpretation }) => {
    ...
    return (
        <Card 
            ref={ref} 
            className={`bg-gradient-to-br from-purple-950/20 via-transparent to-transparent transition-all duration-1000 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}
        >
